...
- 当前完整后端已部署到Ubuntu，数据库迁移到004；API仅绑定127.0.0.1:3000，数据库未映射宿主机端口。生产域名 `https://api.dsbyte.xyz`、HTTPS/WSS代理和微信合法域名已配置，健康检查、REST鉴权与WSS Upgrade已通过外部验证。
- 2026-09-02上线前代码审查确认 `miniprogram/` 无 `wx.cloud`、`cloud://` 和 CloudBase SDK，新后端生产依赖也不包含云开发 SDK；根测试88项、后端70项通过，生产live/ready均返200，未登录本人接口返401。正式发布、多账号弱网、真实 PostgreSQL 多连接并发、备份恢复、留存策略和上线观察仍未完成。旧线上小程序继续使用云开发，未停用或删除旧云服务和数据。
...
这是一个正在从微信云开发迁往自建后端的小程序，用于创建牌局房间、多人实时记分、结算并保存战绩。当前仓库同时保留旧云函数基线和已部署但尚未正式发布的小程序 REST/WSS 实现：
...
用户已确认 Ubuntu 22.04 / 杭州 / 2 vCPU / 2 GiB、旧业务数据无需迁移；ICP备案、HTTPS/WSS生产代理、微信合法域名和完整后端部署已完成，但小程序尚未正式发布切换。旧数据无需迁移不代表授权删除旧数据或关闭云服务。
已确认选型、当前验收缺口和切换顺序见 docs/self-hosted-migration.md，旧版实现细节不机械沿用；遗留功能问题、真实环境完整验收和正式切换仍需完成。角色配置不要求启动全部 Agent，实际委派须有明确任务、授权和文件分工。
