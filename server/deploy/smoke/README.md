...
服务器最初只部署过提交 `0f0a086` 的基础身份后端。2026-08-31已在独立目录 `~/gameintegrator-smoke-20260831/server` 部署当前完整版本：数据库迁移到004、重新授权、重建API并加入头像卷；公网IP的Nginx HTTP代理仅用于开发调试，仍不是完整小程序上线。
...
- 2026-08-31外部确认80端口live返回200，WSS无会话握手返回401；宿主机实际仍只监听127.0.0.1:3000，数据库无宿主机监听。不要开放3000或5432。
- 真实开发者工具联调确认微信登录、资料、普通/奖池房间、2笔下注、领奖池、结算和战绩接口成功；`validate-deployment.sql` 确认房间/战绩平账、结算快照一致、活动关联有效。API单容器重启后数据保留且WSS自动重连。
- 迁移前与联调后均创建了服务器本地、权限600的自定义格式`pg_dump`；联调后备份SHA-256为`f72942c49e1ea6f3c4d38f0f7e0522b5adab51cb36e788b04fd25643b234510f`。当时头像卷为空；尚未完成恢复演练，不能把备份文件或命名卷本身当成已验证恢复能力。
