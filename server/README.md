...
当前已有配置校验、PostgreSQL迁移、微信登录与会话、本人资料修改、头像上传与鉴权读取、创建/加入/退出房间、房主转移、房间快照、转分/批量转分、下注/底注/All-in/领奖池、持久化幂等、计分流水分页、房主结算/解散、战绩列表/详情/头像权限、按需生成微信小程序码、同端口WebSocket实时同步、注销、限流与健康检查。小程序代码已替换为自建REST/WSS适配层，生产地址已配置为 `https://api.dsbyte.xyz`；证书、健康检查、REST鉴权及WSS Upgrade已通过外部验证，微信合法域名、生产域名下的小程序码/新扫码入口及多账号弱网仍待验收。
...
2026-09-02上线前代码审查回归：新后端70项、根小程序/旧云端兼容及独立性门禁88项全部通过，全70个JavaScript文件通过 `node --check`，`git diff --check` 通过。独立性门禁扫描小程序发布运行码中的 `wx.cloud`、`cloud://` 和 CloudBase SDK，并确认新后端生产依赖不含云开发 SDK。其余回归覆盖身份与越权、迁移升级、资料/头像、房间生命周期、计分与奖池、持久化幂等、故障回滚、结算/解散、战绩、二维码、WSS和客户端原请求重试。

这些结果仍不等于完整生产验收：PGlite中的并发提交由单连接适配器排队，真实PostgreSQL 17多连接脚本尚未运行；本地自动测试中的微信登录和二维码仍使用替身，生产域名真机验证也尚有部分功能问题，多账号弱网和备份恢复仍未验收。
...
2026-08-31已部署当前完整镜像并迁移到004，重启后ready成功；头像卷实际写入通过，未登录接口返回401。公网IP的80端口Nginx测试代理已启用，外部live返回200，WSS无会话握手返回401；API仍仅映射127.0.0.1:3000，数据库没有宿主机端口。

随后真实开发者工具完成微信登录、普通/奖池房间、下注、领奖池、结算和战绩联调；API重启后WSS自动重连且数据保留。`deploy/smoke/validate-deployment.sql` 只输出汇总与一致性结果，可用于确认房间/战绩平账、结算快照及活动关联，不输出玩家身份或具体分数。
...
- `Dockerfile` 提供应用镜像定义，可通过 `NODE_IMAGE` 指定已验证的官方基础镜像；`NPM_REGISTRY` 默认仍是npm官方仓库，服务器无法连接时可在构建命令临时指定可信镜像。生产TLS代理已经启用；正式发布、备份、告警及切换方案仍待完成。
...
- 更新服务器时，先由建表账号运行全部迁移，再执行当前 `deploy/smoke/grant-app.sql`，然后构建/重建API并验证。运行账号需要房间及成员表读写权限；回执表仅SELECT/INSERT，不授予UPDATE/DELETE。ready会检查完整表访问；不迁移或不授权不能启动为就绪状态。2026-08-31隔离环境已按此流程部署到004，但小程序尚未正式切换，也无需更新旧云函数。
...
计分使用 `003_score_ledger.sql`，保留001/002原文件，升级已有房间时不改积分、席位或回执；新增奖池为0、底注及下注记录为null。完整部署必须继续执行下节004迁移。应用对流水表仅有SELECT/INSERT，不能直接UPDATE/DELETE；删除整房间时数据库按既有外键级联清理。ready检查新表、列和可读权限。隔离环境已部署到004；不需上传旧云函数，小程序代码已接新接口但尚未配置生产域名或发布。
...
`miniprogram/config.js` 已固定生产HTTPS根地址 `https://api.dsbyte.xyz`，WSS自动使用 `wss://api.dsbyte.xyz/api/v1/ws`；备案前的公网IP、环境分支和本地存储覆盖均已移除。微信公众平台的request、socket、uploadFile、downloadFile合法域名已配置，证书链、健康检查、REST鉴权与WSS Upgrade已通过外部验证。

Nginx把REST和WSS转发到同一个回环端口，当前生产站点配置见 `deploy/nginx-api.dsbyte.xyz.conf`。API端口不能绕过Nginx访问且代理会覆盖转发头，因此部署时应将 `deploy/compose.proxy.yaml` 与主Compose一起使用，把 `TRUST_PROXY_HOPS`设为1。
...
- `TRUST_PROXY_HOPS=0` 时忽略转发头；当前生产代理会覆盖转发头，应用端口仅绑定回环地址，部署通过 `deploy/compose.proxy.yaml`设为1。
