...
- 用户已确认选型并授权分阶段开发，当前进入**第一批后端基础实现**：server/配置、数据库迁移、微信登录与会话、测试和说明。未授权连接生产服务器、改DNS、正式部署或停用旧云服务。
- “旧数据不迁移”不代表获准删除旧云数据库、文件或停用云服务；本次不做这些操作。
- 现有线上实现仍使用微信云开发。根 AGENTS.md 的云开发说明和演进路线描述既有状态/设想；本次新增自建迁移方向，不表示已经完成切换。
- 已确认Node.js 24/CommonJS、Fastify、PostgreSQL 17、WebSocket、Nginx和Docker Compose。用户要求优化原先受云开发限制的功能，不必坚持旧实现；后续优化与范围见 [迁移计划](../../docs/self-hosted-migration.md)。server/第一批已有实现，但小程序没有切换，也没有部署。
...
待确认：服务商、磁盘和带宽、域名值与备案进度、服务器现有服务、预计在线人数、备份预算与上线窗口。不要读取密钥文件来补这些缺口。
