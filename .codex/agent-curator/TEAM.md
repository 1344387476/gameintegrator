...
- 用户已确认选型并完成当前完整代码阶段：`server/` 已覆盖身份、资料/头像、房间、计分、结算/战绩、二维码和单实例WSS，小程序已接REST/WSS。当前进入提交审查后的部署准备与真实环境验收；未授权改DNS、正式部署或停用旧云服务。
- “旧数据不迁移”不代表获准删除旧云数据库、文件或停用云服务；本次不做这些操作。
- 线上仍使用微信云开发；仓库中的新版小程序代码尚未配置生产API地址或发布。已有Ubuntu隔离环境只运行基础后端旧镜像，不表示完整版本已部署或切换。
- 已确认Node.js 24/CommonJS、Fastify、PostgreSQL 17、WebSocket、Nginx和Docker Compose。当前未完成项和切换顺序见 [迁移计划](../../docs/self-hosted-migration.md)；真实微信、真机、PostgreSQL多连接、备份恢复和生产代理仍需验收。
...
待确认：带宽、域名值与备案进度、预计在线人数、备份存放位置/预算、数据留存规则和上线窗口。已知宿主机存在80端口Nginx，合并代理前须核对用途。不要读取密钥文件来补这些缺口。

## 当前配置：三个专业角色

| 实际来源角色 → 项目角色名 | 为什么需要 / 当前交付 | 文件责任与介入条件 | 验收及退出 |
| --- | --- | --- | --- |
| Backend Architect → `pac-engineering-backend-architect` | 后端接口、数据模型、事务账本、推送边界；交付接口映射和失败恢复方案 | 按任务参与server/基础开发及后续账本设计；阅读三组云函数、app/room页及tests，仅写明确分配的后端/数据库模块，不与前端和运维争用 | 安全整数、负分、成员领奖池、去重、并发与版本校正均有明确验证方法；交付后等待授权 |
...
| DevOps Automator → `pac-engineering-devops-automator` | 单机2GiB与备案约束现在影响设计；交付资源预算、托管对比、备份和切换检查表 | 按任务维护开发配置和运维文档，生产方案在后续阶段完善，服务器操作另需授权 | 解释启动/重启/证书/恢复/切换验证方式；没有实测不报容量、部署完成或零停机 |
...
- Backend Architect：去掉大规模、多地域、自动扩容、默认双写/回填、固定延迟及可用率目标；改为当前规模的事务一致性、维护成本和单后端切换约束。后续用户已授权基础开发及云限制优化，阶段与上限描述已同步更新；不是重新安装团队。
